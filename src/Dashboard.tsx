...
const CARD_ACCENT: Record<'category' | 'unqualified' | 'certify' | 'health', string> = {
...
/** Small uppercase card header with a colored accent dot — used across the chart row for a consistent premium look. */
...
function StatTile({
  ...
  return (
    <div className="rounded-2xl border border-hairline bg-white p-4 shadow-[0_1px_2px_rgba(14,31,23,0.04)] transition-shadow hover:shadow-[0_4px_16px_rgba(14,31,23,0.07)]">
      <div className="mb-3 flex items-center gap-2">
        <span className="flex h-8 w-8 flex-none items-center justify-center rounded-full bg-mint-100 text-mint-600">
          <Icon path={icon} />
        </span>
 ...
 * Transaction Process tabs and the chart row all write into the same filter
 * state (stage + category), so any combination drives the table beneath them.
 */
export function Dashboard({ onOpenLead }: { onOpenLead: (lead: Lead) => void }) {
  ...
  /** Category donut — combines with whatever stage is already selected, if any. */
  function toggleCategoryOnly(key: LeadCategory) {
    setCategoryFilter((cur) => (cur === key ? null : key))
  }

  /** "Not yet qualified" and "At Certify" donuts each own the stage filter, so selecting a slice scopes the table to that stage too. */
  function toggleCategoryWithStage(key: LeadCategory, stage: 'lead' | LeadAction) {
    if (categoryFilter === key && stageFilter === stage) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-hairline bg-white p-4 shadow-[0_1px_2px_rgba(14,31,23,0.04)] transition-shadow hover:shadow-[0_4px_16px_rgba(14,31,23,0.07)]">
          <CardHeader accent={CARD_ACCENT.category}>Customer Category</CardHeader>
        ...
        <div className="rounded-2xl border border-hairline bg-white p-4 shadow-[0_1px_2px_rgba(14,31,23,0.04)] transition-shadow hover:shadow-[0_4px_16px_rgba(14,31,23,0.07)]">
          ...
          <div className="flex justify-center py-1">
            <Donut
              ...
              onSelect={(k) => toggleCategoryWithStage(k as LeadCategory, 'lead')}
        ...
        <div className="rounded-2xl border border-hairline bg-white p-4 shadow-[0_1px_2px_rgba(14,31,23,0.04)]">
      ...
      <div className="mb-4 rounded-2xl border border-hairline bg-white p-4 shadow-[0_1px_2px_rgba(14,31,23,0.04)]">
        <CardHeader accent="var(--color-ipi-800)">Transaction Process (LOA) — click a stage</CardHeader>
        ...
        <div className="mt-2.5 text-[11px] text-ipi-700/45">→ SAP (Accounts) owns final invoicing &amp; collections from Certify onward.</div>
      </div>

      <div className="rounded-2xl border border-hairline bg-white shadow-[0_1px_2px_rgba(14,31,23,0.04)]">
