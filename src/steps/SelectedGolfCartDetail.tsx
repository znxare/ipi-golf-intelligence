import { calculateSelectedGolfCartTotal, deriveActiveGolfCartBrands, deriveVerifiedQty, isGolfCartItemEnabled } from '../calc/commercial'
 ...
 * shows lines matching an active brand — same scope as the priced total.
 */
export function SelectedGolfCartDetail({ lines }: { lines: Record<string, EquipmentVerificationLine> }) {
  ...
  const selected = GOLF_CART_CATALOG.filter((item) => isGolfCartItemEnabled(item, activeBrands))
